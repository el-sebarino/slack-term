...
// Lobby is the definition of a Lobby component
type Lobby struct {
	List     *termui.List
...
// CreateLobbyComponent is the constructor for the Lobby struct
func CreateLobbyComponent(inputHeight int) *Lobby {
	lobby := &Lobby{
		List:     termui.NewList(),
	...
	lobby.List.Height = termui.TermHeight() - inputHeight
	lobby.List.Overflow = "wrap"

        lobby.SetBorderLabel("Lobby")

	return lobby
}

// Buffer implements interface termui.Bufferer
func (c *Lobby) Buffer() termui.Buffer {
	...
	// When we encounter a newline or, are at the bounds of the lobby view we
	...
	// newlines or were at the bounds of the lobby view
	lines = append(lines, line)
...
func (c *Lobby) GetHeight() int {
	return c.List.Block.GetHeight()
}

// SetWidth implements interface termui.GridBufferer
func (c *Lobby) SetWidth(w int) {
	c.List.SetWidth(w)
}

// SetX implements interface termui.GridBufferer
func (c *Lobby) SetX(x int) {
	c.List.SetX(x)
}

// SetY implements interface termui.GridBufferer
func (c *Lobby) SetY(y int) {
	c.List.SetY(y)
}

// GetMaxItems return the maximal amount of items can fit in the Lobby
// component
func (c *Lobby) GetMaxItems() int {
...
// Lobby view
func (c *Lobby) SetMessages(messages []Message) {
...
func (c *Lobby) IsNewThread(parentID string) bool {
...
func (c *Lobby) ClearMessages() {
	c.Messages = make(map[string]Message)
}

// ScrollUp will render the lobby messages based on the Offset of the Lobby
// pane.
//
// Offset is 0 when scrolled down. (we loop backwards over the array, so we
// start with rendering last item in the list at the maximum y of the Lobby
// pane). Increasing the Offset will thus result in substracting the offset
// from the len(Lobby.Messages).
func (c *Lobby) ScrollUp() {
...
// ScrollDown will render the lobby messages based on the Offset of the Lobby
// pane.
//
// Offset is 0 when scrolled down. (we loop backwards over the array, so we
// start with rendering last item in the list at the maximum y of the Lobby
...
// SetBorderLabel will set Label of the Lobby pane to the specified string
func (c *Lobby) SetBorderLabel(conversationName string) {
	c.List.BorderLabel = conversationName
}

// MessagesToCells is a wrapper around MessageToCells to use for a slice of
// of type Message
func (c *Lobby) MessagesToCells(msgs map[string]Message) []termui.Cell {
...
func (c *Lobby) MessageToCells(msg Message) []termui.Cell {
	cells := make([]termui.Cell, 0)
